...
const CONVERSATION_PERMS = ['conversations:read', 'conversations:write'];

function pgArray(values: string[]): string {
  return `ARRAY[${values.map((v) => `'${v}'`).join(', ')}]::text[]`;
}

export class BackfillConversationRolePermissions1700000014001 implements MigrationInterface {
  name = 'BackfillConversationRolePermissions1700000014001';

  async up(q: QueryRunner): Promise<void> {
    for (const role of ['owner', 'admin']) {
      const rows = await q.query(`SELECT permissions FROM roles WHERE is_system = true AND name = '${role}' LIMIT 1`);
      if (!rows[0]) continue;
      const perms = [...new Set([...(rows[0].permissions ?? []), ...CONVERSATION_PERMS])];
    ...
    if (memberRows[0]) {
      const perms = [...new Set([...(memberRows[0].permissions ?? []), 'conversations:read'])];
